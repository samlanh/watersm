<?php 
class Dailywork_Form_FrmWorkcomplete extends Zend_Form
{
	protected $tr;
	...
	public function add($data=null){
		$_workcomplete= new Zend_Dojo_Form_Element_TextBox('workcomplete');
		$_workcomplete->setAttribs(array('dojoType'=>'dijit.form.TextBox','required'=>'true','missingMessage'=>'Invalid Module!','class'=>'fullside'
				
		));
		$_idea= new Zend_Dojo_Form_Element_TextBox('idea');
		$_idea->setAttribs(array('dojoType'=>'dijit.form.TextBox','required'=>'true','missingMessage'=>'Invalid Module!','class'=>'fullside'
		
		));
		if(!empty($data)){
			//print_r($data); exit();
			$_workcomplete->setValue($data['workcompleted']);
			$_idea->setValue($data['noteandidea']);
			
			
		}
		$this->addElements(array($_idea,$_workcomplete));
		return $this;
