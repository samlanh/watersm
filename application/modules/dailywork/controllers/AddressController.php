<?php
class Dailywork_AddressController extends Zend_Controller_Action
{
public function init()
    ...
    {
    	$db =new Dailywork_Model_DbTable_DbAddress();
...
//     		print_r($search); exit();
    	...
    	$rows = $db->getAddress($search);
    	$this->view->row_rs = $db->getAddress();
    	//print_r($db); exit();
    	$list = new Application_Form_Frmlist();
    	$columns=array("contactname" , "contactnumber","category" ,"datecreat", "address" ,"note" ,"wherenote" ,"status");
    	$link=array(
    			'module'=>'dailywork','controller'=>'address','action'=>'edit',
    	);
    	//$urlEdit = BASE_URL ."/product/index/add-brand";
    	$this->view->list=$list->getCheckList(0, $columns, $rows, array('contactname'=>$link,'contactnumber'=>$link,'wherenote'=>$link));
 		$db =new Dailywork_Model_DbTable_DbAddress();
 		$dd=$db->getAddress();
 		//print_r($dd); exit();
       $this->view->rst=$dd;
       
       $dd=$db->getCategoryName();
       //print_r($dd); exit();
       $this->view->rs=$dd;
	...
	public function addAction()
	{
		if ($this->getRequest()->isPost()){
			try {
				...
				$address = new Dailywork_Model_DbTable_DbAddress();
				$address->add($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/address/index");
					
				}else {
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/address/add");
				}
			}
			catch (Exception $e){
		...
		$fm=new Dailywork_Form_FrmAddress();
		$frm_address=$fm->add();
		Application_Model_Decorator::removeAllDecorator($frm_address);
		$this->view->frm_address= $frm_address;
...
// 	}// select Produuct 
	public function editAction(){
		...
		//print_r($id); exit();
    	$db = new Dailywork_Model_DbTable_DbAddress();
    	$dd= $this->view->row_rs=$db->getaddressById($id);
    	//print_r($dd); exit();
    	...
    	if ($this->getRequest()->isPost()){
			try {
				...
				$data['id']=$id;
				$address = new Dailywork_Model_DbTable_DbAddress();
				$address->updateaddressById($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/address/index");
