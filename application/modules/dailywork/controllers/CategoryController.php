<?php
class Dailywork_CategoryController extends Zend_Controller_Action
{
public function init()
    ...
    {
    	$db =new Dailywork_Model_DbTable_DbCategory();
    	...
    	$rows = $db->getCategory($search);
    	$this->view->row_rs = $db->getCategory();
    	//print_r($db); exit();
    	$list = new Application_Form_Frmtable();
    	$columns=array("name" ,"status");
    	$link=array(
    			'module'=>'dailywork','controller'=>'category','action'=>'edit',
    	);
	...
	public function addAction()
	{
		if ($this->getRequest()->isPost()){
			try {
				...
				$category = new Dailywork_Model_DbTable_DbCategory();
				$category->add($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complte", "/dailywork/category/index");
				}else{
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/category/add");
				}
			}
			catch (Exception $e){
		...
		$fm=new Dailywork_Form_FrmCategory();
		$frm_category=$fm->add();
		Application_Model_Decorator::removeAllDecorator($frm_category);
		$this->view->frm_category= $frm_category;
	...
	public function editAction(){
    	...
    	$db = new Dailywork_Model_DbTable_DbCategory();
    	$dd= $this->view->row_rs=$db->getcategorytById($id);
    	if ($this->getRequest()->isPost()){
    		$data = $this->getRequest()->getPost();
    		$data['id']=$id;
    		$category = new Dailywork_Model_DbTable_DbCategory();
    	 	$category->updatecategoryById($data);
    	 	Application_Form_FrmMessage::Sucessfull("EDIT_SUCCESS", "/dailywork/category");
    	}
    	$fm=new Dailywork_Form_FrmCategory();
    	$frm_category=$fm->add($dd);
    	Application_Model_Decorator::removeAllDecorator($frm_category);
    	$this->view->frm_category= $frm_category;
   }
