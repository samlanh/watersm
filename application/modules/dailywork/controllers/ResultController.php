<?php
class Dailywork_ResultController extends Zend_Controller_Action
{
public function init()
    ...
    {
    	$db =new Dailywork_Model_DbTable_DbResult();
...
//     		print_r($search); exit();
    	...
    	$rows = $db->getResult($search);
    	$this->view->row_rs = $db->getResult();
    	//print_r($db); exit();
    	$list = new Application_Form_Frmtable();
    	$columns=array("name","email","result");
    	$link=array(
    			'module'=>'dailywork','controller'=>'result','action'=>'edit',
    	);
	...
	public function addAction()
	{
		if ($this->getRequest()->isPost()){
			try {
				...
				$result = new Dailywork_Model_DbTable_DbResult();
				$result->add($data);
				if(isset($data['save_close'])){
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/result/index");
				}else{
					Application_Form_FrmMessage::Sucessfull("Complete", "/dailywork/result/add");
				}
			}
			catch (Exception $e){
		...
		$fm=new Dailywork_Form_FrmResult();
...
// 	}// select Produuct 
	public function editAction(){
    	...
    	$db = new Dailywork_Model_DbTable_DbResult();
    	$dd= $this->view->row_rs=$db->getresultById($id);
    	if ($this->getRequest()->isPost()){
			try {
				$data = $this->getRequest()->getPost();
				$data['id']=$id;
