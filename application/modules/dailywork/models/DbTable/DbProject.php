<?php
class Dailywork_Model_DbTable_DbProject extends Zend_Db_Table_Abstract
{
    protected $_name = 'tb_project';
 ...
 public function updateprojectById($_data){
 	$_arr = array(
 			'projectname'=>$_data['project'],
 			'date'=>date("Y-m-d",strtotime($_data['start_date'])),
 			'createdate'=>date("Y-m-d"),
 ...
 {
 	$db = $this->getAdapter();
 	$sql = "SELECT id , projectname , date , status  FROM tb_project WHERE id = $id";
...
public  function getProject($search=null){
  $db = $this->getAdapter();
  $sql = " SELECT id, projectname ,date, 
	(SELECT name_en FROM `tb_view`
	WHERE`tb_view`.`key_code`=`tb_project`.`status` AND type=5 )AS status 
  ...
  if(!empty($search["projectname"])){
  	$where.=' AND id='.$search["projectname"];
  }
